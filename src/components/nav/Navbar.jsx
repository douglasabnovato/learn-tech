  ...
  return (
    <div
      ...
      className={`w-full h-[8ch] backdrop-blur-sm flex items-center justify-between md:px-16 sm:px-10 px-4 fixed top-0 transition-all ease-in-out duration-300 z-50 ${
        isScrolled
          ? "bg-sky-50/30 border-b border-neutral-200"
          : "bg-transparent"
      }`}
    >
      {/* Logo */}
      ...
      >
        ...
        <div className="flex-1 flex flex-col md:flex-row items-center justify-between gap-6 p-6 md:p-0">
            ...
            {navItems.map((item) => (
              <li key={item.id}>
                <Link
                  to={item.path}
                  onClick={closeNavbar}
                  className={`hover:text-sky-700 ease-in-out duration-300 ${
          ...
          <div className="flex flex-col md:flex-row items-center gap-4">
            <Link
              to="/signin"
              className="w-fit px-6 py-2 rounded-full md:text-base text-xl text-neutral-800 hover:text-transparent hover:bg-clip-text hover:bg-gradient-to-r from-indigo-500 via-sky-700 to-purple-700 font-medium bg-transparent transition-colors duration-200 cursor-pointer"
            >
              Sign In
            </Link>
